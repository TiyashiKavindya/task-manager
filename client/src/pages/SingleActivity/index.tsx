...
function SingleActiviy() {
    ...
    const dateEl = (start_date: string, end_date: string) => {
        if (start_date && end_date) {
            return (
                <p className="text-sm text-center md:text-left text-gray-300">{convertDateFormat(start_date, '/')} to {convertDateFormat(end_date, '/')}</p>
            )
        } else if (start_date) {
            return (
                <p className="text-sm text-center md:text-left text-gray-300">{convertDateFormat(start_date, '/')}</p>
            )
        } else if (end_date) {
            return (
                <p className="text-sm text-center md:text-left text-gray-300">{convertDateFormat(end_date, '/')}</p>
            )
        }
    }
    ...
    return (
        <>
            ...
            <Scrollable>
                <div className="flex flex-col md:flex-row gap-2 justify-between items-center text-light">
                    <div className="mb-2">
                        <h1 className="flex-grow text-center md:text-left text-xl font-bold line-clamp-2 text-ellipsis mb-1 text-light">{activity.title}</h1>
                ...
                <div className="mt-6 py-2 flex justify-between items-center border-b border-gray-600">
                    <p className='text-lg'>Related Tasks</p>
                    <div className="flex items-center md:items-start flex-col gap-1">
                        <p className="text-xs text-gray-300">Assigned tasks</p>
                        <h1>{tasks.length} Tasks</h1>
                    </div>
                </div>
                <div className="mt-6 flex min-h-10 gap-6 max-w-[275px] sm:max-w-[350px] md:max-w-[600px] lg:max-w-[1000px] xl:max-w-[1200px] overflow-x-scroll no-scrollbar">
                    <button
